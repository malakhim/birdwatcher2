		...
		if ($price == 0) {
			...
			if (file_exists($add_path) == true) {
				...
				while ($add_data = fgetcsv ($add_fp, $max_line_size, $delimiter)) {
					...
					if (!empty($shipping_settings['swisspost'][$add_rates_data['service']]) && $shipping_settings['swisspost'][$add_rates_data['service']] == 'Y' && $add_rates_data['price'] > 0 && strpos($add_rates_data['service'], $type) === 0 && (($add_rates_data['service'] == 'pp_cash_on_delivery' && strpos($code, '_ec_') > 0) || ($add_rates_data['service'] != 'pp_cash_on_delivery'))) {
