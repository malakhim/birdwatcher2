...
function fn_google_sitemap_clear_url_info()
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE') {
		if (defined('COMPANY_ID')) {
			$company = Registry::get('s_companies.' . COMPANY_ID);			
	...
	if (!empty($storefront_url)) {
		$search = array('[http_location]', '[customer_index]');
		$replace = array($storefront_url,  Registry::get('config.customer_index'));
		$sitemap_available_in_customer = str_replace($search, $replace, fn_get_lang_var('sitemap_available_in_customer'));
	} else {
		$sitemap_available_in_customer = '';
	}

	$search = array('[http_location]', '[admin_index]', '[sitemap_available_in_customer]');
	$replace = array($storefront_url, fn_get_index_script(), $sitemap_available_in_customer);
...
HEAD;
	...
	fn_mkdir(DIR_CACHE_MISC . 'google_sitemap/');
	$file = fopen(DIR_CACHE_MISC . 'google_sitemap/' . 'sitemap' . $file_counter . '.xml', "wb");
	...
	if (Registry::is_exist("addons.news_and_emails") && $sitemap_settings['include_news'] == 'Y') {
	...
	if ($file_counter == 1) {
		fn_rename(DIR_CACHE_MISC . 'google_sitemap/' . 'sitemap' . $file_counter . '.xml', DIR_CACHE_MISC . 'google_sitemap/' . 'sitemap.xml');
	} else {
...
HEAD;
		$file = fopen(DIR_CACHE_MISC . 'google_sitemap/' . 'sitemap.xml', "wb");
	...
	if (!empty($map_page)) {
		$name = DIR_CACHE_MISC . 'google_sitemap/' . 'sitemap' . $map_page . '.xml';
		if (file_exists($name)) {
			$filename = $name;
		}
	}
...
function fn_google_sitemap_check_counter(&$file, &$link_counter, &$file_counter, $links, $header, $footer)
{
	...
	if ((count($links) + $link_counter) > MAX_URLS_IN_MAP || $stat['size'] >= MAX_SIZE_IN_KBYTES * 1024) {
		...
		fclose($file);
		$file_counter++;
		$filename = DIR_CACHE_MISC . 'google_sitemap/' . 'sitemap' . $file_counter . '.xml';
