...
function fn_price_list_xls_url_info()
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && !defined('COMPANY_ID')) {
		$text = fn_get_lang_var('ult_xml_info') . '<br />';
	} else {
	...
	$text .= fn_get_lang_var('xls_currencies_note') . '<br />';
	$text .= '<br />';
...
function fn_price_list_pdf_url_info()
{
	$return_url = fn_url('addons.manage', 'A', defined('HTTPS')? 'https' : 'http');
	if (PRODUCT_TYPE == 'ULTIMATE' && !defined('COMPANY_ID')) {
		$text = fn_get_lang_var('ult_pdf_info') . '<br />';
	} else {
...
function fn_build_link_on_price($link,  $mode, $return_url, $lang_code = CART_LANGUAGE)
{
	...
	if (PRODUCT_TYPE == 'ULTIMATE' && !defined('COMPANY_ID')) {
		...
		foreach(fn_get_all_companies_ids() as $company_id){
			...
			$text .= '<li><a href="' . $_link . '&sl=' . CART_LANGUAGE . '&return_url=' . urlencode($return_url) . '">' . $_link . '</a></li>';
		}
		$text .= '</ul>';
	} else {
		$_link = fn_url($link, 'C', 'http');
		$text .= '<a href="' . $_link . $mode . '&sl=' . CART_LANGUAGE . '&return_url=' . urlencode($return_url) . '">' . $_link . $mode . '</a><br />';
	}
