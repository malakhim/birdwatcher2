...
if ($mode == 'delete') {
	if (PRODUCT_TYPE == 'ULTIMATE' && defined('COMPANY_ID')) {
		$company_condition = db_quote("WHERE company_id =?i", COMPANY_ID);
...
//
	...
	$total = db_get_field("SELECT COUNT(*) FROM ?:stat_requests WHERE sess_id = ?i $company_condition ORDER BY req_id", $_REQUEST['stat_sess_id']);
	$limit = fn_paginate($params['page'], $total, Registry::get('settings.Appearance.admin_elements_per_page'));

	$requests = db_get_array("SELECT req_id, url, title, timestamp, https FROM ?:stat_requests WHERE sess_id = ?i $company_condition ORDER BY req_id DESC $limit", $_REQUEST['stat_sess_id']);
		...
		if ($report == 'search') {
			...
			if (!empty($data['data'])) {
				foreach ($data['data'] as $k => $v) {
					...
					$url = "products.search&" . $data['data'][$k]['url'];
					if (!empty($v['storefront'])) {
						$data['data'][$k]['storefront_url'] = (defined('HTTPS') ? 'https://' : 'http://') . $v['storefront'] . '/' . Registry::get('config.customer_index') . '?dispatch=' . str_replace('?', '&', $url);;
					} else {
						$url = fn_url($url, 'C', 'rel', '&');
...
function fn_stat_get_visitors($params, $extra_join, $condition)
{
	...
	$data = db_get_hash_array(
			...
			. (empty($sorting) ? '' : "ORDER BY $sorting") ." $limit", 
			'sess_id'
		);
...
{
	if (PRODUCT_TYPE == 'ULTIMATE' && !empty($data['storefront'])) {
