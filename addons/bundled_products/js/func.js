function fn_bundled_products_get_price_schema(chain_id)
{
	var result = {};
	var prices = {};
	var total_price = 0;
	
	elms = $('div#products_' + chain_id);
	...
	$('.cm-chain-' + chain_id, elms).each(function(){
		...
		if (elm_id != '{bp_id}') {
			...
			prices[elm_id]['price'] = parseFloat($('#item_price_bp_' + chain_id + '_' + elm_id, elms).val());
			prices[elm_id]['modifier'] = parseFloat($('#item_modifier_bp_' + chain_id + '_' + elm_id, elms).val());
			
			if (isNaN(prices[elm_id]['modifier'])) {
				prices[elm_id]['modifier'] = 0;
			}
			
			prices[elm_id]['modifier_type'] = $('#item_modifier_type_bp_' + chain_id + '_' + elm_id, elms).val();
...
function fn_bundled_products_apply_discount(chain_id)
{
	...
	price_schema = fn_bundled_products_get_price_schema(chain_id);
	...
	for (i in prices) {
		...
		$('#item_modifier_bp_' + chain_id + '_' + i, elms).val(discount);
		$('#item_modifier_type_bp_' + chain_id + '_' + i, elms).val('by_fixed');
		$('[id*=item_display_price_bp_' + chain_id + '_' + i + '_]', elms).text(prices[i]['price'].toFixed(2));
		$('[id*=item_discounted_price_bp_' + chain_id + '_' + i + '_]', elms).text(item_price);
...
function fn_bundled_products_recalculate(chain_id)
{
	var prices = {};
	var total_price = 0;
	var discounted_price = 0;
...
function fn_bundled_products_share_discount(evt, chain_id)
{
	if (evt.keyCode) {
		code = evt.keyCode;
	} else if (evt.which) {
	...
	if (code == 13) {
		fn_bundled_products_apply_discount(chain_id);
	}
...
// Hook add_js_item
function fn_bundled_products_add_js_item(data)
{
	if (data['var_prefix'] == 'p') {
		...
		data['append_obj_content'] = data['append_obj_content'].str_replace('{bp_id}', data['item_id']['product_id']).str_replace('{price}', price);
		
		// Price replacement
		var content = $('<tr>' + data['append_obj_content'] + '</tr>');

		content.find('span[id*=\'price_bp\']').each(function() {
...
// Hook add_js_item
function fn_bundled_products_transfer_js_items(data)
{
	for (id in data) {
		...
		if (data[id].option && data[id].option.path) {
			...
			for (option_id in data[id].option.path) {
				...
				modifier = parseFloat($('#bp_option_modifier_' + option_id + '_' + variant_id).val());
				if (!isNaN(modifier)) {
...
$(window).load(function(){
	$('.cm-buy-together-submit').closest('form').bind('submit', function(){
		...
		if (fields.length > 0) {
			$.showNotifications({'data': {'type': 'E', 'title': lang.error, 'message': lang.bundled_products_fill_the_mandatory_fields, 'save_state': false, 'message_state': 'I'}});
		}
	});
});