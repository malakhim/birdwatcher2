	...
	if ($mode == 'update') {
		if (!empty($cart['products'])) {
			foreach ($cart['products'] as $_id => $product) {
				if (!empty($product['extra']['bundled_products']) && !empty($product['prev_cart_id']) && $product['prev_cart_id'] != $_id) {
					foreach ($cart['products'] as $aux_id => $aux_product) {
						if (!empty($aux_product['extra']['parent']['bundled_products']) && $aux_product['extra']['parent']['bundled_products'] == $product['prev_cart_id']) {
							$cart['products'][$aux_id]['extra']['parent']['bundled_products'] = $_id;
