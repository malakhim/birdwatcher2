...
function fn_data_feeds_get_product_features($product_id, $field, $lang_code)
{
	...
	$result = false;
	if (!empty($feature_id)) {
		$variants = db_get_array('SELECT var_descr.variant AS variant, IF(feature_val.value_int IS NULL, feature_val.value, feature_val.value_int) as value FROM ?:product_feature_variant_descriptions AS var_descr RIGHT JOIN ?:product_features_values AS feature_val ON (feature_val.variant_id = var_descr.variant_id) WHERE feature_val.feature_id = ?i AND feature_val.product_id = ?i AND feature_val.lang_code = ?s GROUP BY var_descr.variant_id', $feature_id, $product_id, $lang_code);

		if (!empty($variants)) {
			$vals = array();
			foreach ($variants as $variant) {
				$vals[] = empty($variant['variant']) ? $variant['value'] : $variant['variant'];
			}
			$result = implode(', ', $vals);
		}
	}
