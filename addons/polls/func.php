 ...
 */
function fn_polls_update_page_post($page_data, $page_id, $lang_code = DESCR_SL)
{
...
function fn_polls_delete_page($page_id)
{
	if (!empty($page_id)) {
		...
		Bm_Block::instance()->remove_dynamic_object_data('polls', $page_id);
...
function fn_polls_get_votes($params, $items_per_page = 0)
{
	...
	if (isset($answer_id)) {
		$total = db_get_field("SELECT COUNT(DISTINCT(?:polls_votes.vote_id)) FROM ?:polls_votes LEFT JOIN ?:polls_answers USING(vote_id) WHERE ?:polls_votes.page_id = ?i AND ?:polls_answers.item_id = ?i AND ?:polls_answers.answer_id = ?i" . ($completed ? " AND ?:polls_votes.type = 'C'" : ''), $params['poll_page_id'], $item_id, $answer_id);
	} else {
		$total = db_get_field("SELECT COUNT(vote_id) FROM ?:polls_votes WHERE ?:polls_votes.page_id = ?i" . ($completed ? " AND ?:polls_votes.type = 'C'" : ''), $params['poll_page_id']);
	}
	...
	if (isset($answer_id)) {
		$votes = db_get_hash_array("SELECT ?:polls_votes.vote_id, ?:polls_votes.type, ?:polls_votes.time, ?:polls_votes.ip_address, ?:polls_votes.user_id, ?:users.firstname, ?:users.lastname FROM ?:polls_answers LEFT JOIN ?:polls_votes USING(vote_id) LEFT JOIN ?:users USING(user_id) WHERE ?:polls_votes.page_id = ?i AND ?:polls_answers.item_id = ?i AND ?:polls_answers.answer_id = ?i" . ($completed ? " AND ?:polls_votes.type = 'C'" : '') . " ORDER BY time DESC $limit", 'vote_id', $params['poll_page_id'], $item_id, $answer_id, $answer_id);
	} else {
		$votes = db_get_hash_array("SELECT ?:polls_votes.vote_id, ?:polls_votes.type, ?:polls_votes.time, ?:polls_votes.ip_address, ?:polls_votes.user_id, ?:users.firstname, ?:users.lastname FROM ?:polls_votes LEFT JOIN ?:users USING(user_id) WHERE ?:polls_votes.page_id = ?i" . ($completed ? " AND ?:polls_votes.type = 'C'" : '') . " ORDER BY time DESC $limit", 'vote_id', $params['poll_page_id']);
	}
