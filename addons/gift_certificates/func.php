 ...
 * @param type $cart_products cart products
...
function fn_gift_certificate_notification(&$gift_cert_data, $force_notification = array())
{
	...
	if ($notify_user == true && $gift_cert_data['email'] && $gift_cert_data['send_via'] == 'E') {
		...
		$gift_cert_data['http_location'] = Registry::get('config.http_location');
		$gift_cert_data['company_name'] = Registry::get('settings.Company.company_name');
		...
		fn_send_mail($gift_cert_data['email'], Registry::get('settings.Company.company_orders_department'), 'addons/gift_certificates/gift_certificate_subj.tpl', 'addons/gift_certificates/gift_certificate.tpl', array(), CART_LANGUAGE, '', true, $gift_cert_data['company_id']);
...
function fn_get_gift_certificates($params, $items_per_page = 0)
{
	...
	$total = db_get_field("SELECT COUNT(*) FROM ?:gift_certificates WHERE 1 ?p", $condition . fn_get_gift_certificate_company_condition('?:gift_certificates.company_id'));
	$limit = fn_paginate($params['page'], $total, $items_per_page);

	$gift_certificates = db_get_array(
		"SELECT ?p  FROM ?:gift_certificates WHERE 1 ?p ORDER BY ?p ?p",
		implode(',', $fields), $condition . fn_get_gift_certificate_company_condition('?:gift_certificates.company_id'), $sort, $limit
	);
	...
	fn_view_process_results('gift_certificates', $gift_certificates, $params, $items_per_page);
...
function fn_gift_certificates_get_google_add_items(&$_items, $cart, $_currency)
{
	...
	if (!empty($cart['use_gift_certificates'])) {
		foreach ($cart['use_gift_certificates'] as $gc_code => $gc_data) {
			$_items .= "<item>" .
							"<item-name>" . fn_get_lang_var('gift_certificate') . "(" . $gc_code . ")" . "</item-name>" .
							"<item-description></item-description>" .
	...
	if (!empty($cart['gift_certificates'])) {
		foreach ($cart['gift_certificates'] as $gc_id => $gc_data) {
			$cart_products[$gc_id] = array(
				...
				'product' => fn_get_lang_var('gift_certificate'),
