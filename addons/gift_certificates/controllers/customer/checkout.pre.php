...
if (in_array($mode, array('cart', 'checkout', 'place_order')) && isset($_SESSION['cart']['use_gift_certificates'])) {
	...
	if ($removed_codes) {
		$str_codes = implode(', ', array_keys($remove_codes));
		fn_set_notification('W', fn_get_lang_var('warning'), str_replace('[codes]', $str_codes, fn_get_lang_var('warning_gift_cert_deny')), 'K');
	}
