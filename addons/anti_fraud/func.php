...
function fn_anti_fraud_place_order($order_id, &$action, &$order_status)
{
	...
	if (floatval($risk_factor) >= floatval($af_settings['anti_fraud_risk_factor'])) {
		...
		fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('antifraud_failed_order'));
	} else {
...
function fn_anti_fraud_add_status()
{
	...
	CSettings::instance()->update_value('antifraud_order_status', fn_update_status('', $status_data, 'O'), 'anti_fraud');
}
