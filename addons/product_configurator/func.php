...
{
	$product = Registry::get('view')->get_var('product');
...
function fn_pconf_compare_order_items($item1, $item2)
{
	...
	if (isset($item1['extra']['configuration_id']) && isset($item2['extra']['parent']['configuration']) && 
		$item1['extra']['configuration_id'] == $item2['extra']['parent']['configuration']) {
		$result = -1;
	} elseif (isset($item2['extra']['configuration_id']) && isset($item1['extra']['parent']['configuration']) && 
		$item2['extra']['configuration_id'] == $item1['extra']['parent']['configuration']) {
		$result = 1;
	} else {
...
function fn_product_configurator_get_order_info(&$order, &$additional_data)
{
	if (!empty($order['items'])) {
		uasort($order['items'], 'fn_pconf_compare_order_items');
	}
		...
		if (!empty($product_id) && !empty($product_data['company_id'])) {
			...
			if ($product_data['company_id'] != $product_company_id) {
				...
				if ($is_in_conf) {
					...
					fn_set_notification('W', fn_get_lang_var('warning'), fn_get_lang_var('pconf_company_update_denied'));
				}
			}
		}
	}
...
{
	if (!$result && PRODUCT_TYPE == 'ULTIMATE' && defined('COMPANY_ID')) {
		if (!empty($product['extra']['parent']['configuration'])) {
			...
			if (!empty($parent_id)) {
				...
				if ($product_company_id == COMPANY_ID || fn_ult_is_shared_product($parent_id, COMPANY_ID) == 'Y') {
					$result = true;
				}
			}
		}
...
{
	if (PRODUCT_TYPE == 'ULTIMATE' && !empty($value)) {
