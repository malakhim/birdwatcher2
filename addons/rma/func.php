...
function fn_send_return_mail(& $return_info, & $order_info, $force_notification = array())
{
	...
	if ($notify_user == true || $notify_department == true || $notify_supplier == true) {
		...
		if ($notify_user == true) {
			...
			fn_send_mail($order_info['email'], array('email' => $company['company_orders_department'], 'name' => $company['company_name']), 'addons/rma/slip_notification_subj.tpl', 'addons/rma/slip_notification.tpl', '', $order_info['lang_code']);
		}

		if ($notify_supplier == true) {
			if (PRODUCT_TYPE == 'PROFESSIONAL' && Registry::get('settings.Suppliers.enable_suppliers') == 'Y') {

				fn_companies_suppliers_rma_notification($order_info, $return_info);

			} elseif (PRODUCT_TYPE == 'MULTIVENDOR' && !empty($order_info['company_id'])) {
				$company_language = fn_get_company_language($order_info['company_id']);
				Registry::get('view_mail')->assign('manifest', fn_get_manifest('customer', $company_language, $order_info['company_id']));
				// Translate descriptions to admin language
				Registry::get('view_mail')->assign('return_status', fn_get_status_data($return_info['status'], STATUSES_RETURN, $return_info['return_id'], $company_language));
				fn_send_mail($company['company_orders_department'], Registry::get('settings.Company.company_orders_department'), 'addons/rma/slip_notification_subj.tpl', 'addons/rma/slip_notification.tpl', '', $company_language, $order_info['email']);
			}
		}
...
function fn_rma_recalculate_order($order_id, $recalculate_type, $return_id, $is_refund,  $ex_data)
{
	...
	if (PRODUCT_TYPE == 'MULTIVENDOR') {
		...
		$cart = & $_SESSION['cart'];
		fn_mve_place_order($order_id, 'save', $status_order, $cart);
	}
