...
{if $view_mode != "list"}
	...
	{if !$no_container}<div class="buttons-container">{/if}{if $picker_view}[{/if}
		{include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="news.picker?display=`$display`&amp;picker_for=`$picker_for`&amp;extra=`$extra_var`&amp;checkbox_name=`$checkbox_name`&amp;data_id=`$data_id`"|fn_url but_text=$but_text|default:$lang.add_news but_role="add" but_rev="content_`$data_id`" but_meta="cm-dialog-opener"}
	{if $picker_view}]{/if}{if !$no_container}</div>{/if}

	<div class="hidden" id="content_{$data_id}" title="{$but_text|default:$lang.add_news}">
	</div>
