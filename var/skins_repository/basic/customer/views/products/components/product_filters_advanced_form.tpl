	...
	<td width="33%">
		...
		{elseif $filter.feature_type == "O" || $filter.feature_type == "N" || $filter.feature_type == "D" || $filter.condition_type == "D" || $filter.condition_type == "F"}
			...
			{/if}
				...
				{/if}
				<p{if !$filter.slider} class="select-field"{/if}><input type="radio" name="variants[{$el_id}]" id="no_ranges_{$el_id}" value="" checked="checked" class="radio" /><label for="no_ranges_{$el_id}">{$lang.none}</label></p>
				{if !$filter.slider}
					{foreach from=$filter.ranges item="range"}
...
<div class="buttons-container">
	...
	&nbsp;{$lang.or}&nbsp;&nbsp;<a class="text-button nobg cm-reset-link">{$lang.reset_filter}</a>
</div>
