...
<tr>
	<td valign="top">
		...
		<p class="sku{if !$product.product_code} hidden{/if}" {if !$no_ids}id="sku_{$obj_id}"{/if}>{$lang.sku}: {if !$no_ids}<span id="product_code_{$obj_id}">{/if}{$product.product_code}{if !$no_ids}</span>{/if}</p>
		{/if}
