	...
	<tr valign="top" class="no-border">
		...
		<td class="center">&nbsp;{$oi.amount}
			{if $settings.General.use_shipments == "Y" && $oi.shipped_amount > 0}
					<p><span class="small-note">(<span>{$oi.shipped_amount}</span>&nbsp;{$lang.shipped})</span></p>
				{/if}
		</td>
		...
		{/if}
		<td class="right">&nbsp;<span>{include file="common_templates/price.tpl" value=$conf_subtotal|default:0}</span></td>
	</tr>
