...
<div class="form-field">
	<label>&nbsp;</label>
	<p>
	<a onclick="$.ajaxRequest('{"payment_notification.activate?payment=moneybookers&payment_id=`$payment_id`&email="|fn_url:'A':'rel':'&'}' + $('#pay_to_email_{$payment_id}').val() + '&cust_id=' + $('#customer_id_{$payment_id}').val() + '&platform=' + $('#platform_{$payment_id}').val() + '&merchant_firstname=' + $('#merchant_firstname_{$payment_id}').val() + '&merchant_lastname=' + $('#merchant_lastname_{$payment_id}').val(), {ldelim}method: 'GET'{rdelim}); return false;" href="{$index_script}?dispatch=payment_notification.activate&amp;payment=moneybookers&amp;payment_id={$payment_id}&amp;email={$processor_params.pay_to_email|escape:url}&amp;cust_id={$processor_params.customer_id}&amp;platform={$processor_params.platform}">{$lang.activate_moneybookers_merchant_tools}</a>
	</p>
	<p>&nbsp;</p>
	<p>
