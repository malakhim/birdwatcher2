{if $smarty.const.ACTION}
	{assign var="_action" value=".`$smarty.const.ACTION`"}
{/if}
...
<div class="step-container{if $edit}-active{/if}" id="step_two">
	{if $settings.General.checkout_style != "multi_page"}
		<h2 class="step-title{if $edit}-active{/if}{if $complete && !$edit}-complete{/if} clearfix">
			<span class="float-left">{if !$complete || $edit}2{/if}</span>

			
			{if $complete && !$edit}
				{hook name="checkout:edit_link"}
				<span class="float-right">
					{include file="buttons/button.tpl" but_meta="cm-ajax cm-ajax-force" but_href="checkout.checkout?edit_step=step_two&amp;from_step=$edit_step" but_rev="checkout_*" but_text=$lang.change but_role="tool"}
				</span>
				{/hook}
			{/if}
			
			{hook name="checkout:edit_link_title"}
			<a class="title{if $complete && !$edit} cm-ajax cm-ajax-force{/if}" {if $complete && !$edit}href="{"checkout.checkout?edit_step=step_two&amp;from_step=`$edit_step`"|fn_url}" rev="checkout_*"{/if}>{$lang.billing_shipping_address}</a>
			{/hook}
		</h2>
	{/if}

	<div id="step_two_body" class="step-body{if $edit}-active{/if}{if !$edit} hidden{/if} cm-skip-save-fields">
			<form name="step_two_billing_address" class="{$ajax_form} {$ajax_form_force} cm-ajax-full-render" action="{""|fn_url}" method="{if !$edit}get{else}post{/if}">
			...
			{if $edit}
				...
				<div class="checkout-buttons">
					{include file="buttons/button.tpl" but_name="dispatch[checkout.update_steps`$_action`]" but_text=$lang.continue}
				</div>
			{/if}
			</form>
		</div>
