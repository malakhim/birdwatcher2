...
<ul>
	...
	<li>
		...
		{if !$product.extra.bundled_products}
			<strong class="valign">{$product.amount}</strong>&nbsp;x&nbsp;{include file="common_templates/price.tpl" value=$smarty.request.package_price span_id="price_`$key`" class="none"}
		{else}
	...
	{foreach from=$added_products item="_product" key="_key"}
		{if $_product.extra.parent.bundled_products == $key}
			<li>
...
{elseif $product.extra.parent.bundled_products}
	&nbsp;
{elseif $smarty.request.package == "Y" && !$product.extra.bundled_products && $controller == "wishlist"}
	<div class="clearfix">
	{include file="common_templates/image.tpl" image_width="40" image_height="40" images=$product.main_pair show_thumbnail="Y" no_ids=true class="product-notification-image"}
	<div class="product-notification-content">
	<ul>
