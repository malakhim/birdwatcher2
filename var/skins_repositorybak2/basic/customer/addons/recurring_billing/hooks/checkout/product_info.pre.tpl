{if $product.extra.recurring_plan_id}
	<div class="product-list-field clearfix">
		...
		<span>{$product.extra.recurring_plan.name} (<a href="{"products.view?product_id=`$product.product_id`&amp;cart_id=`$key`&amp;combination=`$opt_combination`&amp;return_to=`$smarty.const.MODE`"|fn_url}">{$lang.rb_edit_subscription}</a>)</span>
	</div>
