...
// Edit report
	...
	fn_add_breadcrumb(fn_get_lang_var('reports'), "sales_reports.manage");
...
// Update table
	...
	$view->assign('intervals', db_get_array("SELECT * FROM ?:sales_reports_intervals ORDER BY interval_id"));
	...
	$report_data = fn_get_report_data($_REQUEST['report_id']);
	fn_add_breadcrumb($report_data['description'], "sales_reports.update?report_id=$_REQUEST[report_id]");
...
// Clear table conditions
	...
	fn_set_notification('N', fn_get_lang_var('notice'), fn_get_lang_var('text_conditions_cleared'));
...
function fn_sales_report_update_table($table_data, $table_id = 0, $lang_code = DESCR_SL)
{
	...
	if (empty($table_id)) {
		...
		// Create parameters
		$_data = fn_check_table_fields($table_data['elements'], 'sales_reports_table_elements');
