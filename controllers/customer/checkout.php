...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($mode == 'add_profile') {
		...
		if (list($user_id, $profile_id) = fn_update_user(0, $_REQUEST['user_data'], $auth, false, true)) {
			...
			db_query("DELETE FROM ?:user_session_products WHERE session_id = ?i AND type = ?s AND user_type = ?s", Session::get_id(), 'C', 'U');
	...
	if ($mode == 'update_steps') {
		...
		if (!empty($auth['user_id'])) {
			...
			if ($_REQUEST['update_step'] == 'step_two' || $_REQUEST['update_step'] == 'step_one' && !$errors) {
				...
				list($user_id, $profile_id) = fn_update_user($auth['user_id'], $user_data, $auth, $_REQUEST['ship_to_another'], $send_notification, false);
