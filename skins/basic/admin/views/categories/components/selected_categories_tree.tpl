...
<tr class="table-row">
	...
	<td width="100%" class="no-padding">
		<a href="{categories.update?category_id=`$cur_cat.category_id`"|fn_url}" class="manage-item{if $cur_cat.status == "N"}-disabled{/if}">{$cur_cat.category}</a>{if $cur_cat.status == "N"}&nbsp;<span class="small-note">-&nbsp;[{$lang.disabled}]</span>{/if}
	</td>
