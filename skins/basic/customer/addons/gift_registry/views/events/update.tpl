...
<div class="events">
			...
			<form action="{""|fn_url}" method="post" name="event_form">
				...
				{if $access_key}
					<div class="events-key">
						...
						<strong>{$lang.text_remember_access_key}:</strong>
						<p>{$access_key}</p>
					</div>
				{/if}
				{if $auth.user_id}
				...
				<div class="event-envitees">
					...
					<div id="elm_invitees">
						...
						{if $event_data.subscribers}
							<span class="event-envitees-desc">{$lang.text_delete_recipients}</span>
						{/if}
					</div>
				</div>
				<div class="buttons-container {if $id}events-update{/if}">
					{if $id}
						...
						{include file="buttons/button.tpl" but_text=$lang.delete_this_event but_href="events.delete_event?event_id=$id&amp;access_key=$access_key" but_role="text"}
					{else}
						{include file="buttons/button.tpl" but_text=$lang.event_add but_name="dispatch[events.update]"}
					{/if}
				</div>
			</form>
		</div>
	...
	{capture name="mainbox_title"}
		{if !$id}
			{$lang.add_event}
		{else}
			{$lang.editing_event}: {$event_data.title}
		{/if}
	{/capture}
</div>