...
<div class="filters-tools clearfix">
	...
	<a href="{if $smarty.request.category_id}{assign var="use_ajax" value=true}{"categories.view?category_id=`$smarty.request.category_id`"|fn_url}{else}{assign var="use_ajax" value=false}{$index_script|fn_url}{/if}" rel="nofollow" class="reset-filters{if $allow_ajax && $use_ajax} cm-ajax cm-ajax-full-render" rev="{$ajax_div_ids}{/if}">{$lang.reset}</a>
	{/if}
</div>
