				<ul class="product-filters {if $collapse}hidden{/if}" id="content_{$filter_uid}">
					...
					{foreach from=$filter.selected_ranges name="selected_ranges" item="selected_range"}
						...
						<li>
							{strip}
								...
								<a href="{$href}" class="filter-item checked{if $allow_ajax && $use_ajax} cm-ajax cm-ajax-full-render" rev="{$ajax_div_ids}{/if}" rel="nofollow">{$filter.prefix}{$selected_range.range_name|fn_text_placeholders}{$filter.suffix}</a>
							{/strip}
						</li>
					{/foreach}
					...
					{capture name="view_all"}
						{if $filter.more_cut}
							...
							<li id="view_all_{$filter_uid}" class="right">
					...
					{if $filter.more_ranges|fn_is_not_empty}
						...
						<ul id="more_ranges_{$filter_uid}" {if $more_collapse}class="hidden"{/if}>
							...
							{$smarty.capture.view_all}

						</ul>

						<li class="extra-link-wrap">
							<a id="on_more_ranges_{$filter_uid}" class="extra-link cm-save-state cm-combination-more_{$filter_uid}{if !$more_collapse} hidden{/if}">{$lang.more}</a>
							<a id="off_more_ranges_{$filter_uid}" class="extra-link cm-save-state cm-combination-more_{$filter_uid}{if $more_collapse} hidden{/if}">{$lang.less}</a>
						</li>
					{else}
						{$smarty.capture.view_all}
					{/if}
