...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	if ($mode == 'success') {
		...
		$response_data = Array (
			...
			'project_password' => $processor_data['params']['project_password']
		);
	...
	$post_data = Array (
		'user_id' => $processor_data['params']['user_id'],
		'project_id' => $processor_data['params']['project_id'],
		'sender_holder' => '',
...
<form method="post" action="{$url}" name="process">
	...
	<input type="hidden" name="language_id" value="{$processor_data['params']['language_id']}">

EOT;
$msg = fn_get_lang_var('text_cc_processor_connection');
$msg = str_replace('[processor]', 'DIRECTebanking server', $msg);
echo <<<EOT
	</form>
