...
//Environment
$sandbox = ($processor_data['params']['mode'] == 'test') ? 'base' : 'api';
$post_address = "https://$sandbox.merchantwarrior.com/post/";

//Pass Phrase
$passPhrase = $processor_data['params']['api_passphrase'];
...
$post[] = 'merchantUUID=' . $processor_data['params']['merchant_id'];
$post[] = 'apiKey=' . $processor_data['params']['api_key'];

// General Transaction Params
$post[] = 'transactionCurrency=' . $processor_data['params']['currency'];
$post[] = 'transactionProduct=' . $processor_data['params']['order_prefix'] . (($order_info['repaid']) ? ($order_id . '_' . $order_info['repaid']) : $order_id);
...
//Transaction Hash
$post[] = 'hash=' . md5(fn_strtolower($passPhrase . $processor_data['params']['merchant_id'] . $order_info['total'] . $processor_data['params']['currency']));
...
list($headers, $response_data) = fn_https_request('POST', $post_address, $post);
