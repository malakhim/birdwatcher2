...
if (defined('PAYMENT_NOTIFICATION')) {
	if ($mode == 'notify') {
		...
		fn_order_placement_routines($_SESSION['order_id'], false);
	} elseif ($mode == 'decline') {
		if (!isset($_SESSION['order_id'])) {
			die('Access denied');
		}
		fn_set_notification('E', '', fn_get_lang_var('text_order_placed_error'));
		fn_order_placement_routines($_SESSION['order_id'], false);
	} elseif ($mode == 'refer') {
		if (!isset($_SESSION['order_id'])) {
			die('Access denied');
		}
		fn_set_notification('W', fn_get_lang_var('important'), $processor_response['REFER']);
		fn_order_placement_routines($_SESSION['order_id'], false);
	}
} else {
	$post_url = ($processor_data['params']['mode']=='test') ? 'https://test.pay4later.com/credit_app/' : 'https://secure.pay4later.com/credit_app/';
	$_order_id = ($order_info['repaid']) ? ($order_id . '_' . $order_info['repaid']) : $order_id;
	$order_description = fn_get_lang_var('order') . " #$order_id";
...
<form method="post" action="{$post_url}" name="process">
	<input type="hidden" name="Identification[api_key]" value="{$processor_data['params']['merchant_key']}"/>
	<input type="hidden" name="Identification[RetailerUniqueRef]" value="{$_order_id}"/>
	<input type="hidden" name="Identification[InstallationID]" value="{$processor_data['params']['installation_id']}"/>
	...
	<input type="hidden" name="Finance[Code]" value="{$processor_data['params']['finance_product_code']}"/>
	<input type="hidden" name="Finance[Deposit]" value="{$processor_data['params']['deposit_amount']}"/>
EOT;
$msg = fn_get_lang_var('text_cc_processor_connection');
$msg = str_replace('[processor]', 'Pay4Later server', $msg);
echo <<<EOT
	</form>
