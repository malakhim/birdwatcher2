...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	$result = "&" . fn_sagepay_simplexor(base64_decode(str_replace(' ', '+', $_REQUEST['crypt'])), $processor_data["params"]["password"]) . "&";
	...
	$post_encrypted = 'VendorTxCode=' . $processor_data['params']['order_prefix'] . (($order_info['repaid']) ? ($order_id . '_' . $order_info['repaid']) : $order_id) . '-' . fn_date_format(time(), '%H_%M_%S') . "&";
