...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	if ($mode == 'notify' && !empty($_REQUEST['order_id']) && !empty($_REQUEST['ssl_result_message']) && isset($_REQUEST['ssl_result'])) {
		...
		$pp_response['order_status'] = 'F';
		$pp_response['reason_text'] =  fn_get_lang_var('error');;
	}
	...
	$post_data['ssl_test_mode'] = $processor_data['params']['mode'] == 'live' ? 'FALSE' : 'TRUE';
	...
	$post_data['ssl_error_url'] = Registry::get('config.http_location') . "/$index_script?dispatch=payment_notification.error&payment=virtual_merchant&order_id=$order_id&";
	...
	$post_url = ($processor_data['params']['mode'] != 'demo')? "https://www.myvirtualmerchant.com/VirtualMerchant/process.do" : "https://demo.myvirtualmerchant.com/VirtualMerchantDemo/process.do";
