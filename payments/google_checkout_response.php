...
if (AREA != 'A' && PRODUCT_TYPE == 'ULTIMATE' && !defined('COMPANY_ID') && $google_sess_id) {
	//To correct session start we must define company id
	$company_id = db_get_field("SELECT company_id FROM ?:sessions WHERE session_id = ?s", $google_sess_id);
	fn_define('COMPANY_ID', $company_id);
}
...
elseif ($message_recognizer == 'order-state-change-notification') {
	...
	if ($google_checkout['order_state'] == "CHARGEABLE" && $google_checkout['prev_state'] == "REVIEWING" && $processor_data['params']['gc_auto_charge'] == 'Y') {
		// Charge an Order
...
function fn_fill_google_shipping_info($id, & $cart, $order_adj, $order_shipping)
{
	...
	if(fn_check_suppliers_functionality()) {
		if (strpos($id, '_')) {
			$shipping_ids = explode('_', $id);
		}
		fn_companies_fill_shipping_info($shipping_ids, $cart);
	}

	fn_set_hook('fill_google_shipping_info', $id, $cart, $order_adj, $order_shipping);
