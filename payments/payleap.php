...
$payleap_order_id = $processor_data['params']['order_prefix'] . $payleap_order_id;
...
list($a, $response) = fn_https_request("POST", $base_url, $post_data);
...
if ($xml === false) {
	$pp_response['reason_text'] = fn_get_lang_var('unknown_server_response');
	$pp_response["order_status"] = 'F';
} else {
	$result = (string) $xml->Result;
	$pp_response['host_code'] = (string) $xml->HostCode;
	$pp_response['auth_code'] = (string) $xml->AuthCode;
	$pp_response["reason_text"] = fn_get_lang_var('reason_text') . ': ' . (string) $xml->RespMSG;
...
{
	$date = date('ymd');
	$items = fn_payleap_build_xml_items($order_info['items']);
