...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	if (!empty($_REQUEST['key'])) {
		...
		if ((strtoupper(md5($processor_data['params']['secret_word'] . $processor_data['params']['account_number'] . $order_number_id . $order_info['total'])) == $_REQUEST['key']) && ($_REQUEST['credit_card_processed'] == 'Y')) {
			$pp_response['order_status'] = ($processor_data['params']['fraud_verification'] == 'Y') ? $processor_data['params']['fraud_wait'] : 'P';
		...
		if (fn_check_payment_script('2checkout.php', $_REQUEST['order_id'])) {
			if ($processor_data['params']['fraud_verification'] == 'Y') {
		...
		if (!empty($_REQUEST['vendor_order_id'])) {
			...
			if (!empty($order_id)) {
				...
				if ($_REQUEST['fraud_status'] == 'pass') {
					...
					$pp_response['order_status'] = $processor_data['params']['fraud_fail'];
				}
