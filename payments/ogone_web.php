...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	$post["accepturl"] = $post["declineurl"] = $post["exceptionurl"] = Registry::get('config.current_location') . "/" . INDEX_SCRIPT . "?dispatch=payment_notification.result&payment=ogone_web&order_id=" . $order_id;
	$post["cancelurl"] = Registry::get('config.current_location') . "/" . INDEX_SCRIPT . "?dispatch=payment_notification.cancel&payment=ogone_web&order_id=" . $order_id;

	if (isset($processor_data['params']['use_new_sha_method']) && $processor_data['params']['use_new_sha_method'] == 'Y') {
