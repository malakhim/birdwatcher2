...
if (!empty($order_info['items'])) {
	foreach($order_info['items'] as $k => $v) {
		...
		$post[] = "<price_including_vat>" . fn_format_price(($v['subtotal'] - fn_external_discounts($v) + $product_tax) / $v['amount']) . "</price_including_vat>";
...
if (floatval($order_info['shipping_cost'])) {
	foreach($order_info['shipping'] as $key => $value) {
		$shipping_tax = 0;
		if (!empty($order_info['taxes'])) {
			list($shipping_tax, $tax_percent) = fn_get_pay_read_tax($order_info['taxes'], $k, 'S');
		}
		$post[] = "<freeform_purchase>";
		$post[] = "<line_number>" . ++$i . "</line_number>";
		$post[] = "<description>" . $value['shipping']."</description>";
		$post[] = "<price_including_vat>" . fn_format_price(array_sum($value['rates']) + $shipping_tax) . "</price_including_vat>";
		$post[] = "<vat_percentage>" . $tax_percent . "</vat_percentage>";
		$post[] = "<quantity>1</quantity>";
		$post[] = "</freeform_purchase>";
	}
...
function fn_get_pay_read_tax($order_taxes, $item_id, $type = 'P')
{
	...
	if (!empty($order_taxes)) {
		foreach ($order_taxes as $tax_id => $tax_data) {
			...
			if (isset($tax_data['applies']['items'][$type][$item_id]) && $tax_data['applies']['items'][$type][$item_id] && $tax_data['price_includes_tax'] != 'Y') {
				$tax += (($tax_data['rate_type'] == 'P') ? $tax_data['rate_value'] : 0);
			}
		}
	}
