...
if ($message_recognizer == 'OrderCalculationsRequest') {
	...
	if (AREA != 'A' && PRODUCT_TYPE == 'ULTIMATE' && !defined('COMPANY_ID') && $amazon_sess_id) {
		$company_id = db_get_field("SELECT company_id FROM ?:sessions WHERE session_id = ?s", $amazon_sess_id);
