  ...
  if (is_null($fm)) {
    ...
    if (is_null($fm)) {
      if (!empty($_REQUEST['order_id'])) {
      	$redirect_url = INDEX_SCRIPT . '?dispatch=orders.details&order_id=' . $_REQUEST['order_id'];
      } else {
      	$redirect_url = INDEX_SCRIPT;
      }
      
      fn_set_notification('E', fn_get_lang_var('error'), str_replace('[font]', $fontfile, fn_get_lang_var('font_metrics_error')));
      fn_redirect((defined('HTTPS') ? Registry::get('config.https_location') : Registry::get('config.http_location')) . '/' . $redirect_url);
    };
  }
