...
class Bm_Block extends CompanySingleton {
		...
		if (!empty($status_data['snapping_id']) && !empty($status_data['status'])) {
			if (!empty($status_data['object_type']) && !empty($status_data['object_id']) && $status_data['object_id'] > 0) {
				...
				if (empty($status_data['object_ids'])) {
					...
					$this->update_statuses($status_data);
				}
			} else {
	 ...
	 */
	public function update_statuses($status_data) {
	 ...
	 */
	public function get_all_contents($block_id, $with_dynamic_objects = false) 
	{
		$condition = "";
     ...
     */
    public function is_block_type_active_on_current_location($block_type)
    {
        ...
        $current_location = Bm_Location::instance()->get($dispatch, $dynamic_object);

        if (!empty($current_location['location_id'])) {
            $blocks = $this->get_blocks_by_type_for_location($block_type, $current_location['location_id']);
     ...
     */
    public function get_blocks_by_type_for_location($block_type, $location_id)
    {
        $blocks = false;

        if (!empty($location_id)) {
            $containers = Bm_Container::instance()->get_list_by_area($location_id, 'C');
            $grids = Bm_Grid::get_list(Bm_Container::instance()->get_ids($containers), array('g.grid_id', 'g.container_id'));
