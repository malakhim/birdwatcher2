...
function fn_get_product_data($product_id, &$auth, $lang_code = CART_LANGUAGE, $field_list = '', $get_add_pairs = true, $get_main_pair = true, $get_taxes = true, $get_qty_discounts = false, $preview = false, $features = true, $skip_company_condition = false)
{
	...
	if (!empty($product_id)) {
		...
		if (PRODUCT_TYPE != 'ULTIMATE') {
			...
			if (!$skip_company_condition && defined('COMPANY_ID')) {
				if (AREA == 'C') {
...
function fn_gather_additional_products_data(&$products, $params)
{
	...
	if ($params['get_categories']) {

		$category_condition = fn_get_localizations_condition('?:categories.localization', true);



		$category_condition .= db_quote(' AND products.product_id IN (?a)', $product_ids);

		$category_join_cond = ' AND (' . fn_find_array_in_set($auth['usergroup_ids'], '?:categories.usergroup_ids', true) . ')';
		$category_join_cond .= db_quote(" AND ?:categories.status IN (?a) ", array('A' , 'H'));
		$category_join = " INNER JOIN ?:products_categories as products_categories ON products_categories.product_id = products.product_id INNER JOIN ?:categories ON ?:categories.category_id = products_categories.category_id $category_join_cond ";

		$product_categories = db_get_hash_single_array(
			...
			. $category_join
			. "WHERE 1 $category_condition"
			. "GROUP BY products.product_id",
			array('product_id', 'category_ids')
		);
	}
	 ...
	 * @param string $product_categories String with comma separated list of product categories
	 */
	fn_set_hook('gather_additional_products_data_pre', $product_ids, $params, $products, $auth, $products_images, $product_options, $has_product_options, $has_product_options_links, $product_categories);
	...
	foreach ($products as &$_product) {
		...
		if ($params['get_categories']) {
			if (!empty($product_categories[$product_id])) {
				$product['category_ids'] = $product_categories[$product_id];
			}
		}
		...
		if ($params['get_options']) {
			...
			foreach ($selected_options as $option_id => $variant_id) {
				if (!empty($product['product_options'][$option_id]['disabled'])) {
		...
		if (!empty($product['qty_step'])) {
            ...
            // max amount
            if ($product['tracking'] != 'D' && $allow_negative_amount != 'Y' && Registry::get('settings.General.inventory_tracking') == 'Y') {
            ...
            if (!empty($product['max_qty']) && $product['max_qty'] < $max_qty) {
                $max_qty = fn_floor_to_step($product['max_qty'], $product['qty_step']);
            }

            for ($i = $min_qty; $i <= $max_qty; $i += $product['qty_step']) {
                $qty_content[] = $i;
                //$per_item += $product['qty_step'];
            }
		}
...
function fn_get_categories($params = array(), $lang_code = CART_LANGUAGE)
{
	...
	if (PRODUCT_TYPE == 'MULTIVENDOR') {
		...
		} elseif (isset($params['company_ids']) && !empty($params['company_ids'])) {
...
function fn_delete_product($product_id)
{
	if (!empty($product_id)) {
		...
		// We must delete files before main product, otherwise files were not removed in the MVE, ULT editions.
		$file_ids = db_get_fields("SELECT file_id FROM ?:product_files WHERE product_id = ?i", $product_id);
		if (!empty($file_ids)) {
			foreach ($file_ids as $file_id) {
				fn_delete_product_file($file_id);
			}
		}
...
function fn_update_product($product_data, $product_id = 0, $lang_code = CART_LANGUAGE)
{
    ...
    if (!empty($product_data['list_qty_count']) && $_data['list_qty_count'] < 0) {
        $_data['list_qty_count'] = 0;
    }

    if (!empty($product_data['qty_step']) && $_data['qty_step'] < 0) {
        $_data['qty_step'] = 0;
    }

    if (!empty($product_data['min_qty'])) {
        $_data['min_qty'] = fn_ceil_to_step(abs($product_data['min_qty']), $product_data['qty_step']);
    }

    if (!empty($product_data['max_qty'])) {
        $_data['max_qty'] =  fn_ceil_to_step(abs($_data['max_qty']), $product_data['qty_step']);
    }
		...
		foreach ($product_data['product_features'] as $feature_id => $value) {
			...
			if ($feature_type == 'D') {
				...
				if (!empty($product_data['add_new_variant'][$feature_id]['variant'])) {
					...
					$i_data['value_int'] = $product_data['add_new_variant'][$feature_id]['variant'];
...
function fn_delete_product_file($file_id)
{
	$files_path = db_get_row("SELECT file_path, preview_path, product_id FROM ?:product_files WHERE file_id = ?i", $file_id);
	if (PRODUCT_TYPE == 'MULTIVENDOR' && !fn_company_products_check($files_path['product_id'], true)) {
			return array(CONTROLLER_STATUS_DENIED);
	}
	if (!empty($files_path['file_path'])) {
		if(file_exists(DIR_DOWNLOADS . $files_path['product_id'] . '/' . $files_path['file_path'])) {
			unlink(DIR_DOWNLOADS . $files_path['product_id'] . '/' . $files_path['file_path']);
		}
	}

	if (!empty($files_path['preview_path'])) {
		if(file_exists(DIR_DOWNLOADS . $files_path['product_id'] . '/' . $files_path['preview_path'])) {
			unlink(DIR_DOWNLOADS . $files_path['product_id'] . '/' . $files_path['preview_path']);
		}
	}

	db_query("DELETE FROM ?:product_files WHERE file_id = ?i", $file_id);
	db_query("DELETE FROM ?:product_file_descriptions WHERE file_id = ?i", $file_id);
		...
		if ($params['variants'] == true) {
			foreach ($data as $k => $v) {
				if (in_array($v['feature_type'], array('S', 'M', 'N', 'E'))) {
					...
					list($f_variants, $v_total) = fn_get_product_feature_variants($v['feature_id'], $params['product_id'], $v['feature_type'], true, $lang_code, 1, $variants_per_page);
...
function fn_get_product_features_list($product, $display_on = 'C', $lang_code = CART_LANGUAGE)
{
	...
	$_data = db_get_array("SELECT $fields FROM ?:product_features as f $join WHERE $condition ORDER BY f.position, fd.description, fv.position");
 ...
 */
function fn_get_product_feature_variants($feature_id, $product_id, $feature_type, $get_images = false, $lang_code = CART_LANGUAGE, $page = 1, $items_per_page = 0)
{
	/**
	...
	$feature_id = is_array($feature_id) ? $feature_id : array($feature_id);
	...
	if (!empty($filters)) {
		...
		if (!empty($params['get_variants'])) {
			...
			if (!empty($_ids)) {
				...
				list($variants) = fn_get_product_feature_variants(array_values($_ids), 0, '');
				$_ids_revert = array();
	...
	if (Registry::is_exist($key) == false) {
		...
		if (PRODUCT_TYPE != 'ULTIMATE' && AREA == 'C') {
			if (fn_check_suppliers_functionality()) {
				// if MVE or suppliers enabled
				if (PRODUCT_TYPE == 'MULTIVENDOR' && !empty($_REQUEST['company_id'])) {
...
function fn_add_range_to_url_hash($hash, $range, $field_type = '')
{
	...
	if ($prefix == 'R' || $prefix == 'V') {
		// remove previous diapason
		$hash = fn_delete_range_from_url($hash, $_range, $field_type);
	}
...
function fn_generate_name($str, $object_type = '', $object_id = 0)
{
	...
	*/
    ...
    if (!empty($str)) {
        $convert_letters = fn_get_schema('literal_convertor', 'schema');
	...
	foreach ($templates_path as &$path) {
		...
		if (!empty($view_templates)) {
			foreach ($view_templates as &$file) {
				if ($file != '.' && $file != '..') {
					preg_match("/(.*" . fn_basename($skin_name) . "\/customer\/)(.*)/", $path, $matches);
